...
function Assembler(context) {
  ...
  function makeProxyCallback(callback, componentIsSingleton) {
    return function proxyCallback(err, component) {
      if (err) {
        callback(err);
        return;
      }
      if (componentIsSingleton) {
  ...
  function assembleCallback(callback) {
    try {
      ...
      var componentIsSingleton = targetComponentDefinition.scope === Scopes.singleton;
      var queuedForAssembly = kernelView.isQueuedForAssembly(targetComponentName);

      var proxyCallback = makeProxyCallback(callback, componentIsSingleton);

      if (componentIsSingleton && queuedForAssembly) {
        // wait for queued component to be assembled and 
      ...
      if (dependencyNames.length) {
        ...
        var assemblerList = _.map(dependencyNames, function (componentName) {
              return makeAssembler(componentName);
            });

        // assemble all the dependencies
        // Cannot do this parallelly as there is chance two dependencies who
        // transitively depend on a singleton component, may get two separate
        // copies of the singleton
        async.series(assemblerList, function (err, components) {
          if (err) {
            callback(err);
            return;
          }
