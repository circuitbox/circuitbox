...
function Kernel() {
  ...
  // using two separate instances of buses.
  // internalBus will be used to route internal circuitbox events;
  // containerBus will be used for application events.
  var internalBus = new EventEmitter();
  var containerBus = new EventEmitter();
  ...
  registry.registerModule(function (registry) {
    registry.for('bus').use(containerBus).scope(Scopes.singleton);
  });
